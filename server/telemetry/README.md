...
Servidor de la telemetría opt-in de Retramo (AGENTS.md §10). Netlify Functions + Netlify Blobs.
...
`STATS_TOKEN` es una variable de entorno del proyecto `retramo-telemetry` en Netlify.

```bash
curl -H "Authorization: Bearer $STATS_TOKEN" https://retramo-telemetry.netlify.app/stats
```
